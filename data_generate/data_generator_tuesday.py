...
if __name__ == "__main__":
    ...
    file_path = f'{dataset}_monday.json'
    df1 = pd.read_json(file_path, orient='records', lines=True)
    insert_dataframe(db_connect, dataset, df1)

    file_path = f'{dataset}_tuesday.json'
    df2 = pd.read_json(file_path, orient='records', lines=True)
    insert_dataframe(db_connect, dataset, df2)
